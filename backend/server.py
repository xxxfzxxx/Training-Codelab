...
async def update_bitcoin_price():
    ...
    if price != -1:
        conn.insert_timestamp(BitcoinTimestamp(convert_date_to_text(datetime.now()), price))
...
async def get_bitcoin_prices():
    timestampes = conn.get_all_timestampes()
    data = [timestamp.__dict__ for timestamp in timestampes]
